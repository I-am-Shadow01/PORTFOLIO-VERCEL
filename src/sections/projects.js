...
export function renderProjects({ projects }) {
  ...
  section.innerHTML = `
    ...
    <h2 class="section-title reveal d1">Things I've Built</h2>

    <div class="projects-grid">
      ${projects.map((p, i) => `
        <article class="project-card reveal d${(i % 3) + 1}" data-index="${i}">
            ...
            <div class="project-actions">
              ${p.github ? `
                <a href="${p.github}" target="_blank" rel="noopener noreferrer"
                   class="project-link-btn" title="View on GitHub" aria-label="GitHub repo for ${p.name}">
                  ${GITHUB_ICON}
                </a>` : ''}
              ${p.demo ? `
                <a href="${p.demo}" target="_blank" rel="noopener noreferrer"
                   class="project-link-btn" title="Live Demo" aria-label="Live demo for ${p.name}">
                  ${EXTERNAL_ICON}
                </a>` : ''}
            </div>
          </div>
          ...
          <div class="project-techs" aria-label="Technologies used">
