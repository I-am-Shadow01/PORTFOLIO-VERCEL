...
export function renderHero({ meta, about }) {
  ...
  section.innerHTML = `
    ...
    <div class="hero-content">
      ...
      <div class="hero-tags reveal d3" aria-label="Focus areas">
        ${about.tags.map(t => `<span class="hero-tag">${t}</span>`).join('')}
      </div>
  ...
  // Start after a small delay for visual polish
  setTimeout(tick, 900);

  // Cleanup if section removed
  section.addEventListener('remove', () => clearTimeout(timer));
